...
def call_brief_model(provider: str, model: str, materials: CreativeMaterials, workflow: str, analysis_manifest: dict[str, Any] | None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    payload = {
        "task": "Create a high-quality Musia song production brief. Return strict JSON.",
        "workflow": workflow,
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are Musia Producer: a senior songwriter, arranger, lyricist, and AI music QA engineer. "
...
def write_ace_config(project_dir: Path, materials: CreativeMaterials, brief: dict[str, Any]) -> Path:
    ...
    lines = ["# Generated by Musia Creative Studio"]
...
def render_brief_markdown(project: CreativeProject, brief: dict[str, Any], model_meta: dict[str, Any], analysis_manifest: dict[str, Any] | None) -> str:
    ...
    lines = [
        f"# Musia Creative Project: {materials.title}",
        "",
...
def render_commands(project_dir: Path, ace_config: Path, workflow: str, language: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "    ;;",
            "  ace-tmux)",
            f"    tmux new-session -d -s musia-ace-$(basename {project_dir}) 'cd {ROOT} && {ace_cmd}; read -p \"done\"'",
            "    ;;",
            "  show)",
            ...
            f"    PYTHONNOUSERSITE=1 conda run -n musia python scripts/musia_quality_check.py \"$audio\" --language {language} --expected-lyrics-file {project_dir / 'lyrics_draft.txt'} --output-dir {project_dir}",
            "    ;;",
            "  soulx-demo-en)",
...
def render_aginti_handoff(project: CreativeProject) -> str:
    return f"""# AgInTiFlow Handoff - Musia Creative Project
...
```bash
codex -m gpt-5.5 -c model_reasoning_effort=\"high\" -s danger-full-access -a never --cd {ROOT} \"Inspect {project.root}, run the Musia commands, improve lyrics/prompt quality, generate or review audio outputs, and update the project report with evidence.\"
```
...
def create_project(
    ...
    if analyze_reference and materials.reference_audio and Path(materials.reference_audio).exists():
        ...
        analysis_manifest = run_pipeline(
            ...
            asr_model="small",
            language=None,
            demucs_device=os.getenv("MUSIA_DEMUCS_DEVICE", "cuda"),
        )
    ...
    project.commands = {
        ...
        "web": "scripts/start_musia_studio_tmux.sh",
    }
